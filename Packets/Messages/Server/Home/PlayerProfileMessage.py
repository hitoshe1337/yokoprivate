    ...
    def encode(self):
        for player in self.players:
            if self.low_id == player["lowID"]:
                ...
                self.writeVint(9)
                self.writeVint(player["roborumble"]) # Best robo rumble time

                self.writeVint(10)
                self.writeVint(player["bigbrawler"]) # Best time as big brawler

                self.writeVint(11)
                self.writeVint(player["duoWins"])  # Duo victories
                
                self.writeVint(12)
                self.writeVint(player["bossfightlvl"]) # Highest boss fight lvl passed

                self.writeVint(13)
                self.writeVint(player["ppp"]) # Highest power player points 

                self.writeVint(14)
                self.writeVint(player["powerplayrank"]) # Highest power play rank

                self.writeVint(15)
                self.writeVint(player["challenge"])  # most challenge wins
